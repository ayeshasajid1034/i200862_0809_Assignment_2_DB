...
SELECT CORP_NAME , CORP_ADDRESS
FROM CORPORATION
JOIN AIRPLANE
ON  CORPORATION.CORP_NAME= AIRPLANE.OF_TYPE
WHERE AIRPLANE.STORED_IN > 200;
...
SELECT OWNER_ID, NAME_, ADDRESS_ CORP_NAME, CORP_ADDRESS
FROM OWNS
INNER JOIN CORPORATION ON CORP_ID = OWNER_ID
INNER JOIN PERSON ON PERSON.SSN = OWNER_ID
WHERE OWNS.P_DATE >= DATEADD(month, -1, GETDATE());
...
LEFT JOIN MAINTAIN ON.SSN = EMPLOYEE.SSN
